...
export default function SyllabusPage({ semester, syllabi }: Props) {
    ...
    const filtered = useMemo(() => {
        ...
        return syllabi.filter((item: PublicSyllabus) => {
            const text = [item.course, item.description ?? ''].join(' ').toLowerCase();
    ...
    const breadcrumbs = [
        ...
        { title: semester.name, href: route('syllabus.show', { semester: semester.id }) },
    ];
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-8 py-10">
                ...
                <div className="space-y-4 text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                        Browse every course for {semester.name}. Download the official PDF shared by the admin team.
                    </p>
                ...
                {filtered.length === 0 ? (
                    <p className="text-center text-muted-foreground">No syllabi found for this semester.</p>
                ) : (
                    <div className="grid gap-6 md:grid-cols-2">
                        {filtered.map((syllabus: PublicSyllabus) => (
                            <Card key={syllabus.id} className="border border-gray-200 dark:border-gray-800">
                                ...
                                <CardContent className="flex items-center justify-between">
                                    <span className="text-sm text-muted-foreground">
                                        {formatFileSize(syllabus.file_size)} • {syllabus.file_name}
                                    </span>
                                    <Button variant="outline" size="sm" asChild>
                                        <a href={syllabus.download_url}>
